  ...
  reducers: {
    selectFilter: (state, action: PayloadAction<string>) => {
      ...
      if(action.payload == 'All') {
        ...
        const districtsIds:any = {"Khorog":1, "Darwoz":2, "Vanj":3, "Rushon": 4, "Shughnon":5,"Ishkoshim": 6, "Roshtqala":7, "Murghob":8 }

        state.filteredData = state.projects.filter(
          (project: DataInterface) => project.district_id == districtsIds[action.payload]
        );
      }
...
export const { selectFilter, viewProject, seacrhFilter, projectCardExpanded } = projects.actions;
